//
//  adPhotoProductLinkViewController.swift
//  scyneApp
//
...
class adPhotoProductLinkViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        sameLabel.frame = CGRect(x: 20, y: linkTextView.bottom + 10, width: view.width - 40, height: 40)
        
    }
    
    
    @objc func didTapSame() {
        let vc = adPhotoBottomLinkLabelViewController(company: self.companyName, logo: self.companyLogo, link: self.companyLink, images: self.images, product: self.companyLink)
        navigationController?.pushViewController(vc, animated: true)
    }
    
    @objc func didTapNext() {
        guard let prodLink = linkTextView.text else {return}
        let vc = adPhotoBottomLinkLabelViewController(company: self.companyName, logo: self.companyLogo, link: self.companyLink, images: self.images, product: prodLink)
        navigationController?.pushViewController(vc, animated: true)
    }
